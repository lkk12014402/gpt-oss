...
class QuantLinear(nn.Module):
    ...
    def __init__(
        ...
        # print("/sdp/lkk/gpt-oss-20b/.venv/lib/python3.12/site-packages/auto_round_extension/torch/qlinear_torch_zp.py")
        super().__init__()
    ...
    def forward(self, x):
        ...
        if self.bits in [2, 4, 8]:
            ...
            zeros = torch.bitwise_and(zeros, self.maxq).reshape(self.scales.shape) + 1
        ...
        if hasattr(self, "g_idx"):
            ...
            # repeat_scales = self.scales.repeat_interleave(self.group_size, dim=0)[:self.infeatures, :]
            # repeat_zeros = zeros.repeat_interleave(self.group_size, dim=0)[:self.infeatures, :]
            # repeat_scales = self.scales.repeat_interleave(self.group_size, dim=0).unsqueeze(dim=-1)
            # repeat_zeros = zeros.repeat_interleave(self.group_size, dim=0).unsqueeze(dim=-1)
            repeat_scales = self.scales.T.reshape(-1, 1)
            repeat_zeros = zeros.T.reshape(-1, 1)
            # repeat_scales = self.scales.repeat_interleave(self.group_size, dim=0).T
            # repeat_zeros = zeros.repeat_interleave(self.group_size, dim=0).T
            wtensor, orig_shape, pad_len = reshape_pad_tensor_by_group_size(weight.T, self.group_size)
            # print(repeat_scales.shape)
            # print(wtensor.shape)
            # weights = repeat_scales * (weight - repeat_zeros)
            # print()
            # weights = repeat_scales * (wtensor.reshape(repeat_scales.shape) - repeat_zeros)
            weights = repeat_scales * (wtensor - repeat_zeros)

            # print(weights.shape)
            # weights = revert_tensor_by_pad(weights.reshape(wtensor.shape), orig_shape=orig_shape, pad_len=pad_len).T
            weights = revert_tensor_by_pad(weights, orig_shape=orig_shape, pad_len=pad_len).T
            # print(weights.shape)
            # exit()
