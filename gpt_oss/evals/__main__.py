...
class ParallelSampler:
    ...
    async def __call__(self, eval_obj):
        ...
        for i, sampler in enumerate(self.samplers):
            subset = total_examples[i * chunk_size : (i + 1) * chunk_size]
            print(len(subset))
            # tasks.append(asyncio.to_thread(eval_obj.run_on_subset, sampler, subset))
            # tasks.append(loop.run_in_executor(None, eval_obj.run_on_subset, sampler, subset))
            tasks.append(eval_obj.run_on_subset_async(sampler, subset))
...
async def main():
    ...
    base_urls = args.base_url.split(",")
    print(base_urls)

    models = {}
    for model_name in args.model.split(","):
        for reasoning_effort in args.reasoning_effort.split(","):
            samplers = [sampler_cls(model=model_name, reasoning_model=True, reasoning_effort=reasoning_effort,
                                    temperature=args.temperature, base_url=url, max_tokens=131_072)
                        for url in base_urls]
            print(samplers)

            models[f"{model_name}-{reasoning_effort}"] = ParallelSampler(samplers)
